...
cleanup() {
  rm -f "${KEY_FILE}" "${API_JSON}"
}
trap cleanup EXIT

write_pem() {
  if [[ -n "${ASC_API_KEY_P8_BASE64:-}" ]]; then
    # Strip whitespace/newlines from the base64 blob, then decode.
    local b64
    ...
    # Unity env UI often stores literal \n instead of real newlines.
...
if ! grep -q "BEGIN PRIVATE KEY" "${KEY_FILE}"; then
  ...
  echo "Prefer ASC_API_KEY_P8_BASE64 (one-line base64 of the .p8 file)."
  wc -c "${KEY_FILE}" || true
  exit 1
fi
...
# Fastlane API key JSON (handles PEM with real newlines).
python3 - <<'PY'
import json, os
...
# 1) fastlane pilot (most reliable on UBA Mac images)
if command -v fastlane >/dev/null 2>&1; then
  echo "Trying fastlane pilot upload..."
  if fastlane pilot upload \
    --ipa "${IPA_PATH}" \
    --api_key_path "${API_JSON}" \
    --skip_waiting_for_processing \
    --skip_submission; then
    UPLOAD_OK=1
  else
